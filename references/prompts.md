...
- 零幻觉（可溯源否则省略）：卡片里任何**具体事实**都必须来自搜索结果或用户输入——不只是数字，还包括**联系方式（邮箱/电话/网址/账号）、人名/职衔/机构名、引语及其归属、日期/引用出处**。无可溯源真值的槽位**宁可省略或留显眼占位**（如 `[演讲人邮箱]`），**绝不编造一个像真的假值**（编造联系邮箱是重大事故）。"扩展/补充"只允许重组或凸显资料里**已有**的内容，不得凭空生造新事实。
- 覆盖所有页面（封面到结束页）
```
...
**只渲染策划稿/页面内容里给定的文案，不得自行发明具体事实。** 页脚、页眉、封面署名、结束页的联系方式/品牌名、引语归属等槽位，若上游没有提供真值就**留空省略**——**绝不凭空填一个像真的邮箱、电话、网址、人名或机构名**（编造联系邮箱是重大事故）。缺真值时用显眼占位（如 `[演讲人邮箱]`）标注待填，不要造假值。详见 `references/method.md` §7 可溯源否则省略。
...
| 结束页 | 总结回顾、行动号召 | 3-5 条核心要点回顾 + 明确的 CTA（下一步；联系方式仅在用户提供时才放，不得编造） |
