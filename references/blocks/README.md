...
| **`columnar_panel`（按需注入）** | **并列内容的专业容器** -- 单一外框 + 内部竖线分隔，取代 N 个独立描边卡片 | `columnar-panel.md` |
...
| **2-3 类并列内容（无强立场）** | **columnar_panel**（按需注入 `block_refs:["columnar-panel"]`） | 单一外框 + 内部竖线，比 N 个独立描边卡片干净 10 倍；列头 + section-label + bullet-dot 形成层级 |
| A vs B 对比决策（有推荐立场） | **comparison** | 并排对比让观众"自己得出结论"而非被动接受 |
