...
## Part A-bug：找茬心态与"新鲜眼睛"（每轮扫描前先入戏）

> 借鉴 Anthropic pptx 技能的图审纪律：**第一版渲染几乎从不正确。**

**找茬心态（bug hunt，不是确认）**：

- **假设这一页一定有问题，你的任务是把它找出来** —— 不是确认它没问题。
- **如果第一遍扫描零发现，是你没看够狠**，回去再看一遍，带着"哪里会翻车"的预期看。
- 生成这页 HTML 的是你自己，你会"看到你期望的样子，而不是实际渲染的样子" —— 这正是下面"新鲜眼睛"存在的原因。

**新鲜眼睛（fresh eyes，宿主支持子代理时强制，哪怕只有 2–3 页）**：

把最新截图交给一个**未参与生成、只读的子代理**做冷读；它没有"我刚写过这段 CSS"的思维定势。给它这份检查指令：

```
视觉检查这些幻灯片。假设里面有问题——把它们找出来。
逐张排查：
- 元素重叠（文字穿过色块、线穿过字、元素堆叠）
- 文字溢出或在边缘/容器边界被切断
- 装饰线是按单行标题定位的，但标题换行成了两行 → 线与文字打架
- 来源标注/页脚与上方内容碰撞
- 元素过近（间距 < 24px）或卡片几乎相贴
- 间距不均（一处大片空，一处挤成一团）
- 距画布边缘留白不足（< 40px）
- 文字框过窄导致过度换行
- 浅底浅灰字 / 深底深图标等低对比
- 残留占位文字（xxxx / lorem / "此处放..."）
逐张列出所有问题，哪怕很小。给出每张图的预期，再对照实际。
```

冷读返回的清单并入本轮 Part B 严重度分级。**修一处常引出另一处 —— 每次修完必回到 Part A 重扫，不做完一整轮零新增就不算完。**
...
> **可视化只走 `html2png.py`（file://）。** 本阶段所有"看图"都由下方 `html2png.py` 截图产出——
> 无端口、无 shell、无本地服务器，在 CloudStorage/OneDrive 同步盘上也稳。**禁止**为看渲染效果
> 起 `python3 -m http.server` 或依赖预览型 MCP（如 `mcp__Claude_Preview`）；`open <file>` 只用于
> 把成品交给用户，不作为本阶段的核对手段。
...
### 第 4 遍：视觉一致性扫描（跨维度 + 跨页）

> 物理扫描（前 3 遍）查"这一页有没有坏"；这一遍查"这一页是否自洽、且像同一套 deck"。每个维度配一个 `visual_qa.py` 自动检查 id（客观项）或纯人工判断项（感知项）。自动项先看脚本输出，再人工复核；感知项只能人工看图。

| 维度 | 看什么 | 严重度 | 对应检查 |
|------|--------|--------|---------|
| **调色板贴合** | 着色是否都来自风格调色板？有无突兀的离色块？ | P1 | `PAL-01`（离色像素比） |
| **硬编码颜色** | HTML 里是否出现 `#xxxxxx`/`rgb()` 而非 CSS 变量？ | P1 | `HEX-01` ≡ 既有 **P1-5**（同一缺陷，勿重复记两条；以 P1-5 为准，`HEX-01` 是其自动化镜像） |
| **字号阶梯** | 标题/正文/标注是否拉开层级？有无 15/16/17px 糊作一团？ | P1 | `TYPE-01`（兼顾既有 P2-5 字号梯度） |
| **圆角/描边一致** | 卡片圆角、描边宽度是否统一为 token？ | P2 | `RAD-01` ≡ 既有 **P2-1**（同一缺陷，以 P2-1 为准，`RAD-01` 是其自动化镜像） |
| **间距韵律** | 间距是否落在 8/4px 栅格？同组紧、异组松（接近律）？ | P2 | `ALIGN-01`（间距栅格） |
| **元素尺寸一致** | 同类重复元素（节点/卡/条目）是否等大？（眯眼看视觉重量） | P2 | 人工 |
| **图解主题绑定** | diagram/timeline 的颜色是否绑定主题变量、随风格变色？ | P1 | `DTHEME-01`（图解硬编码检测） |
| **光学对齐** | 圆形/箭头等是否需光学补偿（几何居中≠视觉居中）？ | P2 | 人工 |
| **视觉平衡** | 重量分布是否有意（Z/F/居中），有无单侧失衡？ | P2 | 人工 |
| **跨页归属感** | 标题/页脚位置、背景处理、字号阶梯是否与相邻页一致？看起来像同一套 deck 吗？ | P1 | 末轮跑 `visual_qa.py` 批量模式的 `DECK-PAL-01`/`DECK-BG-01`/`DECK-TYPE-01` |
