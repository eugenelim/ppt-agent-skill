...
- [ ] Route paths for class edges shortened by `MarkerSpec.clearance` at
      the marker end so the marker sits at the card face, not inside it.
      (deferred: class-diagram-marker-clearance)
- [ ] Route entry/exit points clipped to actual class-card bounding rect.
      (deferred: class-diagram-route-clip)
- [ ] Edge labels placed on longest stable segment (≥40 px).
      (deferred: class-diagram-label-segment)
