...
### ELK adapter
- [x] AC-ELK-1: `layout/elk_adapter.py` exports `layout_with_elk(graph: LayoutGraph, spacing=None) -> FinalizedLayout`. `_compile_flowchart` routes through `layout_with_elk` (elk-production-path-unification landed).
...
- [x] AC-VAL-2: `validate_finalized_layout(strict=True)` returns non-pass structural status when any non-cross-hierarchy child (a child whose parent group IS included in the ELK layout call) extends outside its parent group's bounding box. Cross-hierarchy children (edges that cross subgraph boundaries, placed by ELK `hierarchyHandling=INCLUDE_CHILDREN`) are explicitly excluded. ELK group hierarchy serialization via `elk.hierarchyHandling=INCLUDE_CHILDREN` is now active (elk-group-hierarchy-serialization landed); post-hoc bbox expansion in `_strategies.py` is retained as a containment safety net.
...
- [x] AC-FIX-1: The 10 graph-topology fixtures (`flowchart-all-shapes`, `flowchart-arrows-defs`, `flowchart-diamond-branch`, `flowchart-diamond-clipping`, `flowchart-empty-subgraph`, `flowchart-groups-complex`, `flowchart-inner-direction`, `flowchart-parallel-links`, `statediagram-complex`, `statediagram-nested`) produce a `FinalizedLayout` with zero node-node geometry overlaps (≤ 1 px tolerance), no containment violations, and `validate_finalized_layout(strict=True).geometry == "pass"` (full geometry check including edge routing quality). ELK is configured with `BRANDES_KOEPF` placement and `unnecessaryBendpoints=false` for improved routing (elk-edge-routing-quality landed).
- [x] AC-FIX-2: The 5 non-graph-topology fixtures (`architecture-complex`, `class-relationships-all`, `er-cardinality-all`, `er-ecommerce`, `requirement-basic`) produce non-empty SVG output via `to_svg(experimental=True)` and each yields ≥ 1 semantic entity via `_extract_semantic_from_svg`. `requirement.py` now emits `data-node-id` on each node header (elk-non-graph-entity-count landed).
