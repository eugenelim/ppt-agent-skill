...
- Keep all existing Phase 2 checks #1–#8 byte-identical — only #9 is modified
  (field name normalization + archetype gate; the check's intent is unchanged).
...
- [ ] A new section "哲学路由：应用场景与叙事范式" (or equivalent heading) appears after the
...
- [ ] Each Tier 1 row (`pyramid`, `sparkline`, `hybrid`) specifies cover style,
...
- [ ] The Tier 2 row (`reference`) states that `reference` requires at least one
  `reference_runbook` Part and that a persuasion timeline without any `reference_runbook`
  Part falls to the fallback rows.
- [ ] Each Tier 3 row (`status`, `facilitation`, `informational`) specifies cover,
...
- [ ] Two fallback rows exist: one for `叙事结构` unrecognized + persuasion goal apparent
...
- [ ] `§说服型的两条"诚实"约定` gains an additive note after the two existing items
...
- [ ] The `outline.txt` schema header block contains a `叙事范式` field with all seven
  values listed (`pyramid / sparkline / hybrid / reference / status / facilitation /
  informational`), placed after `叙事结构`.
- [ ] The `叙事范式` field description states: derived by outline agent at Step 2 from
...
- [ ] Step 2 of the 5-step methodology gains an addendum instructing the agent to
...
- [ ] `与上一 Part 的关系` format in the per-Part schema block is updated to require a
...
- [ ] A `页目标` claim-shape and concreteness convention block exists for persuasion
...
- [ ] Step 1 (提炼全局核心论点) gains a cover-resonance requirement for persuasion decks:
...
- [ ] Check #9 row is updated: field name changed from `core_thesis` to `核心论点`;
  row includes an archetype gate (`叙事范式 ∈ {pyramid | sparkline | hybrid}`) and a
  note that non-persuasion archetypes skip this check.
- [ ] Check #10 row exists: "标题叙事序列" (仅 `叙事范式 ∈ {pyramid | sparkline | hybrid}`);
...
- [ ] Check #11 row exists: "结论收束" (仅 `论证策略: narrative_driven` 或 `data_driven`
...
- [ ] Check #12 row exists: "论证部 Part 数量" (仅 `叙事范式 ∈ {pyramid | sparkline |
...
- [ ] Checks #13–#15 rows exist with `叙事范式: status` gate: check #13 (opening
...
- [ ] Checks #16–#18 rows exist with `叙事范式: facilitation` gate: check #16
...
- [ ] Checks #19–#21 rows exist with `叙事范式: informational` gate: check #19
...
- [ ] The `## 自审检查清单` heading is updated from `（9项门禁）` to `（21项，按叙事范式适用）`;
  ...
  (#1–12).
- [ ] The checklist header row count (`## 自审检查清单`) reflects the new total (21
  checks in the table, though each deck sees only its applicable subset).

### File 4 — `references/prompts/tpl-outline-orchestrator.md`

- [ ] Line 40 (`禁止在编写阶段就考虑自审的 7 项检查标准`) is updated to count-neutral
  wording (e.g. `禁止在编写阶段就考虑阶段 2 的自审检查标准`) — removing the stale
  hardcoded "7 项" count. No other line in the file is modified.

### Cross-file

- [ ] The seven `叙事范式` values (`pyramid`, `sparkline`, `hybrid`, `reference`,
  `status`, `facilitation`, `informational`) are listed identically wherever all seven
  appear together in the three reference files.
- [ ] No existing Phase 2 check #1–#8 content is modified (only #9 field name and
  gate are updated as specified).
- [ ] Gate tool checks (`smoke_test.py`, `lint_diagram_recipes.py`, `check_skill.py`)
