...
核心方案：Playwright + dom-to-svg
- Playwright 在 headless Chromium 中打开 HTML
- dom-to-svg 直接将 DOM 树转为 SVG，保留 <text> 元素
- 不经过 PDF 中转，文字不会变成 path
- dom-to-svg 预打包为 scripts/vendor/dom-to-svg.bundle.js（采用者无需 npm/esbuild）

安全控制：
- 仅允许 file:// 和 data: 请求（LLM01/ASI05）
- 图片路径限制在 deck root 内（LLM05/CWE-22）
...
from _browser import _setup_page, _within_deck_root, get_browser
...
def _inline_images(page, html_file: Path, deck_root: Path) -> None:
    ...
    This moves the Node-side fs.readFileSync loop into Python.
    """
        ...
        with get_browser() as browser:
            ...
            for html_file in html_files:
                ...
                try:
                    ...
                    # the fixed wait_for_timeout below is the actual render-settle guard.
                    page.goto("file://" + str(html_file), wait_until="networkidle", timeout=30000)
                    page.wait_for_timeout(500)

                    # Python-side image inlining (replaces Node fs.readFileSync loop)
                    _inline_images(page, html_file, deck_root)

                    # Inject the pre-built dom-to-svg bundle
                    page.add_script_tag(path=str(BUNDLE_PATH))

                    # Materialise CSS features dom-to-svg can't read (verbatim JS port)
                    page.evaluate(_PREPROCESS_JS)
                    page.wait_for_timeout(300)

                    # DOM→SVG conversion (verbatim JS port)
                    svg_string = page.evaluate(_DOM_TO_SVG_JS)
