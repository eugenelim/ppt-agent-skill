...
def to_html(src: str, *, theme: Theme = None, width_hint: int = 0) -> str:
    ...
    fragment = _dispatch(src, None, width_hint)
    return render_page(fragment, theme)


def to_svg(src: str, *, theme: Theme = None, width_hint: int = 0) -> str:
    ...
    html = to_html(src, theme=resolved, width_hint=width_hint)
    from .svg import _to_svg_from_html_string

    return _to_svg_from_html_string(html)


def to_png(src: str, *, theme: Theme = None, scale: float = 1.0, width_hint: int = 0) -> bytes:
